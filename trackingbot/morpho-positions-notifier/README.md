...
```bash
morpho-positions-notifier$ sam build
```
...
```bash
morpho-positions-notifier$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
morpho-positions-notifier$ sam local start-api
morpho-positions-notifier$ curl http://localhost:3000/
```
...
```bash
morpho-positions-notifier$ sam logs -n HelloWorldFunction --stack-name morpho-positions-notifier --tail
```
...
```bash
morpho-positions-notifier$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name morpho-positions-notifier
```
